(function (root, factory) {
  ...
  const SKILL_DETAILS = Object.freeze({
    'shadow-assassination':'單體刺殺；流血目標傷害提高，Lv6 暴擊追加副手攻擊。','death-mark':'標記目標5秒，提高自身對其傷害；擊殺縮短冷卻。','lethal-technique':'提高暴擊傷害；背刺暴擊強化下一次主手普通攻擊。','weakness-insight':'攻擊流血目標時提高暴擊率。','corrosive-strike':'直接傷害；無毒時施加1層毒，已有毒時增加1層並刷新全部毒層。','blood-venom-rend':'直接傷害並施加割裂流血；中毒目標承受更高割裂傷害。','venom-mastery':'提高中毒傷害；Lv6 敵人帶3層毒死亡時保存1次瘟疫蔓延。','toxic-blood-symbiosis':'目標同時中毒與流血時提高持續傷害；Lv6 直接暴擊使每個中毒與流血／割裂追加1次傷害。'
  });
  ...
  const VENOM_SKILLS = Object.freeze([
    ...
    passive('toxic-blood-symbiosis','血毒共生',[6,8,10,13,16,25].map((dotDamage,i)=>({dotDamage:dotDamage/100,bonusTickOnDirectCrit:i===5})),'venom')
  ]);
  ...
  return Object.freeze({FIRST_JOB_CHANGE_LEVEL,POISON_ENTRY_RATIO,ADVANCED_CLASSES,AUTO_SKILL_PRIORITY,ASSASSINATION_SKILLS,VENOM_SKILLS,SKILLS,getSkill,getSkills,getEffect,isAdvanced,canAdvance,advance,hasDot,hasBleedingStatus,poisonStacks,getDeathMarkDamageMultiplier,getTargetDefenseReduction,getTargetBonuses,markTarget,resolveMarkedKill,getAutoSkillPriority:orderAutoSkills,resolveBackstabCrit,getBasicExecution,consumeBasic,resolvePlagueDeath,consumePlague,consumeBonusDotTicksOnCrit,clear});
}));
