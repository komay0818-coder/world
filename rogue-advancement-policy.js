(function (root, factory) {
  ...
  const SKILL_DETAILS = Object.freeze({
    'shadow-assassination':'單體刺殺；流血目標傷害提高，Lv6 暴擊追加副手攻擊。','death-mark':'標記目標5秒，提高自身對其傷害；擊殺縮短冷卻。','lethal-technique':'提高暴擊傷害；背刺暴擊強化下一次主手普通攻擊。','weakness-insight':'攻擊流血目標時提高暴擊率。','corrosive-strike':'武器淬毒；期間主手普通攻擊施加中毒，並依命中前毒層追加毒傷。','blood-venom-rend':'直接傷害並施加持續6秒的割裂；中毒目標承受更高割裂傷害。','venom-mastery':'提高中毒傷害與中毒層數上限；Lv6滿6層時進一步提高中毒傷害。','toxic-blood-symbiosis':'目標同時中毒與流血時提高持續傷害；Lv6 直接暴擊使每個中毒與流血／割裂追加1次傷害。'
  });
  ...
  const VENOM_SKILLS = Object.freeze([
    active('corrosive-strike','淬毒',10,[5,5,6,6,7,7].map((duration,i)=>({duration,cooldown:[10,10,10,9,9,9][i],energyCost:25,poisonBonusPerStack:[0,8,10,12,14,16][i]/100,lethalCoating:i===5?.25:0,breakthrough:i===5?'滿6層時，淬毒追加毒傷提高25%':''})),'venom'),
    active('blood-venom-rend','血毒割裂',8,[140,150,160,170,185,200].map((power,i)=>({power:power/100,energyCost:30,ruptureTick:[17,19,21,23,26,30][i]/100,ruptureDuration:6,ruptureTickInterval:2,ruptureEntryRatio:.75,poisonedRuptureBonus:.20,toxicBloodDefense:i===5?.06:0,breakthrough:i===5?'目標同時中毒與割裂時，額外降低6%防禦':''})),'venom'),
    passive('venom-mastery','劇毒精通',[5,8,10,13,16,25].map((poisonDamage,i)=>({poisonDamage:poisonDamage/100,poisonMaxStacks:[4,4,5,5,6,6][i],maxStackPoisonDamage:i===5?.20:0,breakthrough:i===5?'目標達到6層中毒時，中毒傷害額外提高20%':''})),'venom'),
  ...
  const AUTO_SKILL_PRIORITY=Object.freeze({
    ...
    venom:Object.freeze(['corrosive-strike','poison-blade','backstab','blood-venom-rend','shadow-dance'])
  });
  ...
  function getPoisonMaxStacks(member){if(!isAdvanced(member?.progress,'venom'))return 3;return getEffect('venom-mastery',member.progress.skillLevels?.['assassin:venom-mastery'])?.poisonMaxStacks||3;}
  function getPoisonDamageBonus(member,dots){if(!isAdvanced(member?.progress,'venom'))return 0;const effect=getEffect('venom-mastery',member.progress.skillLevels?.['assassin:venom-mastery']);return(effect?.poisonDamage||0)+(poisonStacks(dots)>=6?effect?.maxStackPoisonDamage||0:0);}
  function applyCoating(member,level,now=Date.now()){const effect=getEffect('corrosive-strike',level);member.venomCoatingUntil=now+effect.duration*1000;member.venomCoatingEffect=effect;return effect;}
  function getCoatingExecution(member,currentStacks,now=Date.now()){if(!isAdvanced(member?.progress,'venom')||now>=(member?.venomCoatingUntil||0))return null;const effect=member.venomCoatingEffect||getEffect('corrosive-strike',member.progress.skillLevels?.['assassin:corrosive-strike']);const stacks=Math.max(0,Number(currentStacks)||0),maxStacks=getPoisonMaxStacks(member);return{stacks,maxStacks,applyPoison:stacks<maxStacks,bonusPower:stacks>0?(effect.poisonBonusPerStack||0)*stacks:0,bonusMultiplier:stacks>=6&&maxStacks===6?1+(effect.lethalCoating||0):1};}
  ...
  function resolvePlagueDeath(){return false;}
  function consumePlague(member){if(!member?.plagueSpreadPending)return false;member.plagueSpreadPending=false;return true;}
  function consumeBonusDotTicksOnCrit(member,dots,critical){const e=getEffect('toxic-blood-symbiosis',member?.progress?.skillLevels?.['assassin:toxic-blood-symbiosis']);if(!critical||!e?.bonusTickOnDirectCrit||!hasDot(dots,'poison')||!hasBleedingStatus(dots))return[];const triggered=[];(dots||[]).filter(d=>['poison','bleed','rupture'].includes(d.type)&&d.remaining>0&&!d.toxicBloodBonusTickUsed).forEach(d=>{d.toxicBloodBonusTickUsed=true;triggered.push(d);});return triggered;}
  function clear(member,leaveBattle=false){if(!member)return;['lethalTechniqueUntil','lethalTechniqueDamage','lethalTechniqueOffhand','venomCoatingUntil'].forEach(k=>member[k]=0);member.venomCoatingEffect=null;if(leaveBattle)member.plagueSpreadPending=false;}
  return Object.freeze({FIRST_JOB_CHANGE_LEVEL,POISON_ENTRY_RATIO,ADVANCED_CLASSES,AUTO_SKILL_PRIORITY,ASSASSINATION_SKILLS,VENOM_SKILLS,SKILLS,getSkill,getSkills,getEffect,isAdvanced,canAdvance,advance,hasDot,hasBleedingStatus,poisonStacks,getPoisonMaxStacks,getPoisonDamageBonus,applyCoating,getCoatingExecution,getDeathMarkDamageMultiplier,getTargetDefenseReduction,getTargetBonuses,markTarget,resolveMarkedKill,getAutoSkillPriority:orderAutoSkills,resolveBackstabCrit,getBasicExecution,consumeBasic,resolvePlagueDeath,consumePlague,consumeBonusDotTicksOnCrit,clear});
}));
