(function (root, factory) {
  ...
  function hasDot(dots,type){return(dots||[]).some(dot=>dot?.type===type&&dot.remaining>0);} function poisonStacks(dots){return(dots||[]).filter(d=>d?.type==='poison').reduce((n,d)=>n+Math.max(1,Number(d.stacks)||1),0);}
  function getTargetBonuses(member,dots,state={},hpRatio=1,attackKind='skill',now=Date.now()){const out={damage:0,crit:0,criticalDamage:0,dotDamage:0,defenseReduction:0};const bleeding=hasDot(dots,'bleed')||hasDot(dots,'rupture');const poisoned=hasDot(dots,'poison');if(isAdvanced(member?.progress,'assassination')&&bleeding){const e=getEffect('weakness-insight',member.progress.skillLevels?.['assassin:weakness-insight']);out.crit+=e?.bleedingCrit||0;out.criticalDamage+=e?.bleedingCritDamage||0;}if(state.deathMarkOwner===member?.id&&now<state.deathMarkUntil){out.damage+=state.deathMarkDamage||0;if(hpRatio<.30)out.criticalDamage+=state.deathMarkExecuteCritDamage||0;}if(isAdvanced(member?.progress,'venom')&&poisoned&&bleeding){const e=getEffect('toxic-blood-symbiosis',member.progress.skillLevels?.['assassin:toxic-blood-symbiosis']);out.dotDamage+=e?.dotDamage||0;const rend=getEffect('blood-venom-rend',member.progress.skillLevels?.['assassin:blood-venom-rend']);out.defenseReduction+=rend?.toxicBloodDefense||0;}if(now<(state.dotVulnerabilityUntil||0))out.dotDamage+=state.dotVulnerability||0;return out;}
  ...
  return Object.freeze({FIRST_JOB_CHANGE_LEVEL,ADVANCED_CLASSES,ASSASSINATION_SKILLS,VENOM_SKILLS,SKILLS,getSkill,getSkills,getEffect,isAdvanced,canAdvance,advance,hasDot,poisonStacks,getTargetBonuses,markTarget,resolveMarkedKill,resolveBackstabCrit,getBasicExecution,consumeBasic,resolvePlagueDeath,consumePlague,extendDotsOnCrit,clear});
}));
