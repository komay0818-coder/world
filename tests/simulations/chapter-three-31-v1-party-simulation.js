...
function basic(p, enemies, now, random) {
  ...
  if (p.job === 'hunter' && target.currentHp > 0) hit(p, target, p.attack * Skills.getEffect('hunter', 'wild-bond', 1).companionAttack, random, false, now);
...
function enemyActions(party, enemies, now, random, warriorWeight) {
  for (const e of aliveEnemies(enemies)) {
    ...
    const frontline = Chapter33.frontlineMultiplier(e, enemies);
    if (frontline !== 1) damage = Math.ceil(damage * frontline);
    ...
    if (frontline > 1 && now < (e.warDrumUntil || 0)) {
      p.dualBuffHits = (p.dualBuffHits || 0) + 1;
      p.dualBuffDamage = (p.dualBuffDamage || 0) + damage;
      p.dualBuffMaxDamage = Math.max(p.dualBuffMaxDamage || 0, damage);
    }
    if (e.allOutTriggered) { e.phaseDamage = (e.phaseDamage || 0) + damage; e.phaseHits = (e.phaseHits || 0) + 1; }
...
function simulate(jobs, seed, warriorWeight, gearId, stage, regenAffixCount = null, killHealAffixCount = null, killHealPerAffix = .03, applyKillHealOpportunityCost = false, monsterPool = POOL, encounter = null, eliteLoop = null, experiment = null) {
  ...
  const loopElite = eliteLoop?.elite || CHAPTER_32_ELITE;
  const spawnTemplate = normal => {
    if (!eliteLoop) return normal;
    loopStats.spawns++;
    if (eliteRandom() < eliteLoop.chance) { loopStats.eliteSpawns++; return loopElite; }
    return normal;
  };
  ...
  if (encounter?.chief34) {
    ...
    chief.dismissSummons = now => enemies.filter(e => e.summoned34 && e.currentHp > 0).forEach(e => { e.currentHp = 0; e.despawned34 = true; e.endedAt34 = now; });
  }
  const duration = encounter?.duration || DURATION;
  const trio34Window = []; let trio34WindowDamage = 0;
  for (let step = 0; step < duration / DT; step++) {
    const now = step * DT;
    const takenBefore34 = eliteLoop?.chapter34 ? party.reduce((n, p) => n + p.taken, 0) : 0;
    ...
    if (eliteLoop) {
      ...
      if (alive.some(e => e.warDrum) && alive.some(e => e.charge34) && alive.some(e => e.fervor34)) loopStats.trio34Seconds += DT;
      if (alive.some(e => now < (e.warDrumUntil || 0))) loopStats.warDrumSeconds += DT;
      if (alive.some(e => e.frontline) && alive.some(e => e.rank === 'normal')) {
        loopStats.frontlineSeconds += DT;
        if (alive.some(e => e.rank === 'normal' && now < (e.warDrumUntil || 0))) loopStats.overlapSeconds += DT;
      }
    }
    ...
    if (eliteLoop?.chapter34) {
      while (trio34Window.length && trio34Window[0].now <= now - 1) trio34WindowDamage -= trio34Window.shift().damage;
      if (trioActive34) {
        const damage = party.reduce((n, p) => n + p.taken, 0) - takenBefore34;
        if (damage) { trio34Window.push({ now, damage }); trio34WindowDamage += damage; }
        loopStats.trio34MaxOneSecond = Math.max(loopStats.trio34MaxOneSecond || 0, trio34WindowDamage);
      }
    }
    enemies.forEach((e, i) => {
      if (e.currentHp <= 0 && e.respawnAt === null) {
        if (e.despawned34) return;
        kills++; if (e.initial) initialKills++; e.respawnAt = now + 2;
        if (e.summoned34) e.endedAt34 = now;
        if (eliteLoop?.chapter34) {
          const lifetime = loopStats.killedLifetimes[e.id] ||= { count: 0, seconds: 0 };
          lifetime.count++; lifetime.seconds += now - e.spawnedAt;
          if (e.fervor34Triggered) loopStats.fervor34Triggers++;
        }
...
if (require.main !== module) {
  ...
  for (const [name, jobs] of Object.entries(PARTIES)) for (const mode of ['loop', 'boss']) {
    if (part && part !== (mode === 'loop' ? `loop${name}` : 'boss')) continue;
    for (const cleared of mode === 'loop' ? [10, 15] : [15]) {
      const stage = proportionalStage(cleared, 25, 16, .16);
      const samples = Array.from({ length: RUNS }, (_, i) => simulate(jobs, 0x31c0de + i * 104729 + name.charCodeAt(0), 3, 'B', stage, 0, 1, .02, true, Chapter34.normals,
        mode === 'boss' ? { templates: [Chapter34.boss], duration: 120, stopImmediately: true, chief34: true } : null,
        mode === 'loop' ? { chance: .05, elite: Chapter34.elite, randomOpening: true, chapter34: true } : null));
      const sum = fn => samples.reduce((n, s) => n + fn(s), 0);
      const avg = (list, fn) => list.length ? list.reduce((n, s) => n + fn(s), 0) / list.length : null;
      const ps = (s, key) => s.party.reduce((n, p) => n + (p[key] || 0), 0);
      const sourceDamage = (s, id) => s.party.reduce((n, p) => n + (p.damageTakenByEnemy[id] || 0), 0);
      ...
      if (mode === 'loop') {
        ...
        cells.push({ ...common, fullPartySurvivalRate: sum(s => Number(s.party.every(p => p.alive))) / RUNS, survivalRate: survivors.length / RUNS,
          averageWipeSeconds: avg(wipes, s => s.time), survivorHpPercent: avg(survivors, hp), dps: sum(s => ps(s, 'totalDamage')) / time,
          killsPerMinute: sum(s => s.kills) / time * 60, priestHealingPerMinute: sum(s => ps(s, 'spellHealing')) / time * 60, killHealingPerMinute: sum(s => ps(s, 'killHealing')) / time * 60,
          championsPerMinute: sum(s => s.loopStats.eliteSpawns) / time * 60, championPresence: sum(s => s.loopStats.elitePresentSeconds) / time,
          enemyDamageShares: Object.fromEntries([...Chapter34.normals, Chapter34.elite].map(e => [e.id, sum(s => sourceDamage(s, e.id)) / enemyDamage])),
          warDrumCoverage: sum(s => s.loopStats.warDrumSeconds) / time,
          fervorTriggerRate: sum(s => s.loopStats.fervor34Triggers + s.enemies.filter(e => e.fervor34Triggered && e.respawnAt === null).length) / sum(s => s.loopStats.eliteSpawns),
          killedEnemyMeanLifetime: Object.fromEntries([...Chapter34.normals, Chapter34.elite].map(e => [e.id, sum(s => s.loopStats.killedLifetimes[e.id]?.seconds || 0) / Math.max(1, sum(s => s.loopStats.killedLifetimes[e.id]?.count || 0))])),
          trioPresence: sum(s => s.loopStats.trio34Seconds) / time,
          trioMaxSingleHit: Math.max(...samples.flatMap(s => s.party.map(p => p.trio34MaxHit || 0))),
          trioMaxOneSecondPartyDamage: Math.max(...samples.map(s => s.loopStats.trio34MaxOneSecond || 0)),
          observedEliteRate: sum(s => s.loopStats.eliteSpawns) / sum(s => s.loopStats.spawns)
        });
      } else {
        const wins = samples.filter(s => s.firstClearAt !== null), rage = samples.filter(s => s.enemies[0].rage34);
        const hits = sum(s => s.enemies[0].skillHits || 0);
        const summonStats = threshold => {
          const entered = samples.filter(s => s.enemies[0][`summon${threshold}`]);
          return { triggerRate: entered.length / RUNS, triggerSeconds: avg(entered, s => s.enemies[0][`summon${threshold}`].at),
            lifetimeSeconds: avg(entered, s => { const e = s.enemies.find(e => e.summonStage === threshold); return (e.endedAt34 ?? s.time) - e.spawnedAt; }),
            damagePerRun: sum(s => sourceDamage(s, threshold === 70 ? Chapter34.normals[0].id : Chapter34.normals[3].id)) / RUNS,
            killedRate: entered.filter(s => { const e = s.enemies.find(e => e.summonStage === threshold); return e.currentHp <= 0 && !e.despawned34; }).length / Math.max(1, entered.length),
            maxTriggers: Math.max(...samples.map(s => s.enemies[0][`summon${threshold}`]?.count || 0)) };
        };
        cells.push({ ...common, killRate: wins.length / RUNS, fullPartyKillRate: wins.filter(s => s.party.every(p => p.alive)).length / RUNS,
          averageKillSeconds: avg(wins, s => s.time), wipeRate: wipes.length / RUNS, timeoutRate: samples.filter(s => s.survived && s.firstClearAt === null).length / RUNS,
          hpPercentAtBossDeath: avg(wins, hp), bossDamagePerRun: sum(s => sourceDamage(s, Chapter34.boss.id)) / RUNS,
          summonDamagePerRun: sum(s => ps(s, 'taken') - sourceDamage(s, Chapter34.boss.id)) / RUNS, priestHealingPerRun: sum(s => ps(s, 'spellHealing')) / RUNS,
          heavySlashDamagePerHit: hits ? sum(s => s.enemies[0].skillDamage || 0) / hits : null, heavySlashHitsPerRun: hits / RUNS,
          summon70: summonStats(70), summon40: summonStats(40),
          rageEntryRate: rage.length / RUNS, rageEntrySeconds: avg(rage, s => s.enemies[0].rage34At), rageDurationSeconds: avg(rage, s => s.time - s.enemies[0].rage34At),
          rageStats: rage.length ? { attack: Chapter34.boss.attack * Chapter34.attackMultiplier(rage[0].enemies[0]), speed: Chapter34.boss.speed * Chapter34.speedMultiplier(rage[0].enemies[0], rage[0].time), defense: Chapter34.boss.defense * Chapter34.defenseMultiplier(rage[0].enemies[0]) } : null,
          maxRageTriggers: Math.max(...samples.map(s => s.enemies[0].rage34Count || 0)),
          summonKillHealingTotal: sum(s => s.enemies.reduce((n, e) => n + (e.summoned34 ? e.killHealPaid || 0 : 0), 0)),
          maxEnemies: Math.max(...samples.map(s => s.enemies.length)),
          liveSummonsAfterBossDeath: wins.reduce((n, s) => n + s.enemies.filter(e => e.summoned34 && e.currentHp > 0).length, 0)
        });
      }
    }
  }
  process.stdout.write(`${JSON.stringify({ test: 'Chapter 3-4 complete TEST V1', runsPerCell: RUNS, gear: 'B1-R0; one 2% kill-heal affix with opportunity cost', suppression: 'S16 / 25 facilities', eliteChance: .05, templates: { normals: Chapter34.normals, elite: Chapter34.elite, boss: Chapter34.boss }, cells }, null, 2)}\n`);
} else if (process.argv.includes('--chapter-33-complete')) {
  const partArg = process.argv.find(value => value.startsWith('--part='));
  const part = partArg?.split('=')[1];
  const cells = [];
  for (const [name, jobs] of Object.entries(PARTIES)) {
    for (const mode of ['loop', 'boss']) {
      if (part && part !== (mode === 'loop' ? `loop${name}` : 'boss')) continue;
      for (const cleared of mode === 'loop' ? [8, 12] : [12]) {
        const stage = proportionalStage(cleared, 20, 14, .14);
        const samples = Array.from({ length: RUNS }, (_, i) => simulate(jobs, 0x31c0de + i * 104729 + name.charCodeAt(0), 3, 'B', stage, 0, 1, .02, true, Chapter33.normals,
          mode === 'boss' ? { templates: [Chapter33.boss], duration: 120, stopImmediately: true } : null,
          mode === 'loop' ? { chance: .05, elite: Chapter33.elite, randomOpening: true } : null));
        const sum = fn => samples.reduce((n, s) => n + fn(s), 0);
        const avg = (list, fn) => list.length ? list.reduce((n, s) => n + fn(s), 0) / list.length : null;
        const ps = (s, key) => s.party.reduce((n, p) => n + (p[key] || 0), 0);
        const time = sum(s => s.time), survivors = samples.filter(s => s.survived), wipes = samples.filter(s => !s.survived);
        ...
        if (mode === 'loop') {
          const enemyDamage = sum(s => ps(s, 'taken'));
          const dualHits = sum(s => ps(s, 'dualBuffHits'));
          cells.push({ ...common, fullPartySurvivalRate: sum(s => Number(s.party.every(p => p.alive))) / RUNS, survivalRate: survivors.length / RUNS,
            ...
            priestHealingPerMinute: sum(s => ps(s, 'spellHealing')) / time * 60, killHealingPerMinute: sum(s => ps(s, 'killHealing')) / time * 60,
            centurionsPerMinute: sum(s => s.loopStats.eliteSpawns) / time * 60, centurionPresence: sum(s => s.loopStats.elitePresentSeconds) / time,
            enemyDamageShares: Object.fromEntries([...Chapter33.normals, Chapter33.elite].map(e => [e.id, sum(s => s.party.reduce((n, p) => n + (p.damageTakenByEnemy[e.id] || 0), 0)) / enemyDamage])),
            warDrumCoverage: sum(s => s.loopStats.warDrumSeconds) / time, frontlineCoverage: sum(s => s.loopStats.frontlineSeconds) / time, overlapCoverage: sum(s => s.loopStats.overlapSeconds) / time,
            dualBuffHitMean: dualHits ? sum(s => ps(s, 'dualBuffDamage')) / dualHits : null, dualBuffHitMax: Math.max(...samples.flatMap(s => s.party.map(p => p.dualBuffMaxDamage || 0))),
            observedEliteRate: sum(s => s.loopStats.eliteSpawns) / sum(s => s.loopStats.spawns)
          });
        } else {
          const wins = samples.filter(s => s.firstClearAt !== null), phase = samples.filter(s => s.enemies[0].allOutTriggered);
          const hits = sum(s => s.enemies[0].skillHits || 0);
          cells.push({ ...common, killRate: wins.length / RUNS, fullPartyKillRate: wins.filter(s => s.party.every(p => p.alive)).length / RUNS,
            averageKillSeconds: avg(wins, s => s.time), wipeRate: wipes.length / RUNS, timeoutRate: samples.filter(s => s.survived && s.firstClearAt === null).length / RUNS,
            hpPercentAtBossDeath: avg(wins, hp), bossDamagePerRun: sum(s => ps(s, 'taken')) / RUNS, priestHealingPerRun: sum(s => ps(s, 'spellHealing')) / RUNS,
            heavySlashDamagePerHit: hits ? sum(s => s.enemies[0].skillDamage || 0) / hits : null, heavySlashHitsPerRun: hits / RUNS,
            phaseEntryRate: phase.length / RUNS, phaseEntrySeconds: avg(phase, s => s.enemies[0].allOutAt), phaseDurationSeconds: avg(phase, s => s.time - s.enemies[0].allOutAt),
            phaseDamagePerRun: sum(s => s.enemies[0].phaseDamage || 0) / RUNS,
            maxPhaseTriggers: Math.max(...samples.map(s => s.enemies[0].allOutTriggerCount || 0)),
            phaseStats: phase.length ? { attack: Chapter33.boss.attack * Chapter33.attackMultiplier(phase[0].enemies[0], phase[0].time), speed: Chapter33.boss.speed * Chapter33.speedMultiplier(phase[0].enemies[0]), defense: Chapter33.boss.defense * Chapter33.defenseMultiplier(phase[0].enemies[0]) } : null,
            maxKills: Math.max(...samples.map(s => s.kills)), maxEnemies: Math.max(...samples.map(s => s.enemies.length))
          });
        }
      }
    }
  }
  process.stdout.write(`${JSON.stringify({ test: 'Chapter 3-3 complete TEST V1', runsPerCell: RUNS, gear: 'B1-R0; one 2% kill-heal affix with opportunity cost', suppression: 'S14 / 20 facilities', eliteChance: .05, templates: { normals: Chapter33.normals, elite: Chapter33.elite, boss: Chapter33.boss }, coverageDefinition: 'fraction of observed run time with at least one eligible living enemy affected; overlap requires a normal enemy with both effects', cells }, null, 2)}\n`);
} else if (process.argv.includes('--chapter-33-s14')) {
