...
function simulate(spec,seed,options={}){
  ...
  const petCount=Hunter.getPetCount(member), survival=Number(options.petHpRatio)>0;
  ...
  for(clock=0;clock<=max&&!done;clock+=STEP){member.resourceCurrent=Math.min(ARROWS,member.resourceCurrent+STEP/1000);if(survival){for(const p of member.companions){const s=petStats[p.id];if(!p.alive&&clock>=p.reviveAt){p.alive=true;p.currentHp=p.maxHp*.5;p.nextAttackAt=clock;p.attackCount=0;p.furyHitCount=0;s.revives++;}if(p.alive){const healed=Math.min(p.maxHp-p.currentHp,p.maxHp*.01*STEP/1000);p.currentHp+=healed;s.regen+=healed;s.aliveMs+=STEP;s.hpIntegral+=p.currentHp/p.maxHp*STEP;s.minHp=Math.min(s.minHp,p.currentHp);}else{s.deadMs+=STEP;}}aliveGroupMs[member.companions.filter(p=>p.alive).length]+=STEP;}dots();if(done)break;
