...
function simulate(spec,seed,options={}){
  ...
  const petCount=Hunter.getPetCount(member);for(let i=0;i<petCount;i++)member.companions.push({id:i,nextAttackAt:0,attackCount:0,furyHitCount:0,wildAwakening:0});
  ...
  let target=fresh(options.template||Temple.normals[0],0), kills=0, done=false, lastDeath=0, damageTaken=0;
  ...
  function pets(){
    ...
    member.companions.forEach((pet,i)=>{if(clock<pet.nextAttackAt||done)return;const label=`pet${'ABC'[i]}`, petAttack=ATTACK*bond.companionAttack*(1+b.damage), critical=random()<CRIT+b.crit;
