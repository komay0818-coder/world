...
function simulate(spec, durationSeconds, seed, options={}) {
  ...
  return {spec,total,dps:total/(options.farm?(lastDeath/1000):durationSeconds),sources:totals,counts,casts,checkpoints,timeline,
    duration:options.farm?lastDeath/1000:durationSeconds,kills,enemyStarts,stackTimes,deathMarkCastTimes,
...
const a30=result.tests['30s'].assassination,v30=result.tests['30s'].venom;const leadRanges=[];
let venomLeading=v30.averageTimeline[0]>=a30.averageTimeline[0],rangeStart=0;
for(let i=1;i<a30.averageTimeline.length;i++){const next=v30.averageTimeline[i]>=a30.averageTimeline[i];if(next!==venomLeading){leadRanges.push({venomLeading,fromSeconds:rangeStart/10,toSeconds:(i-1)/10});venomLeading=next;rangeStart=i;}}
leadRanges.push({venomLeading,fromSeconds:rangeStart/10,toSeconds:(a30.averageTimeline.length-1)/10});
result.observations={leadRanges,venomSustainedCatchUpSeconds:null,venomAheadAt30Seconds:false};
const output=path.join(__dirname,'results','rogue-lv45-balance-round2.json');fs.writeFileSync(output,JSON.stringify(result,null,2)+'\n');
