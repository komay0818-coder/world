...
assert.match(script, /PartyPolicy\.scheduleNextAttack\(member, now, member\.attackSpeed \* skillHasteMultiplier \* blessingSpeedMultiplier \* corruptedSwiftnessMultiplier \* runeFrenzyMultiplier \* huntingRhythmMultiplier \* warriorSpeedMultiplier \* galeSpeedMultiplier \* holyStormSpeedMultiplier \* \(1 \+ rogueBonuses\.attackSpeed\) \* \(1 \+ \(desperate\?\.speed \|\| 0\)\), exhaustedMultiplier \* trailSlowMultiplier\)/, 'attack timing includes conditional rogue weakness speed without replacing existing timing modifiers');
assert.match(script, /chooseRandomAliveMember\(battle\.partyMembers/, 'monsters use the shared weighted selection among living party members');
