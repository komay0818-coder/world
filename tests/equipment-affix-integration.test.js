...
assert.match(html, /script\.js\?v=20260922-brokenrock-32-direct-v2/, 'the affix UI renderer uses the current local build');
...
assert.match(source, /target\.currentHp = Math\.max\((?:0|getPlaytestHpFloor\(\)), target\.currentHp - damage\);[\s\S]*?PriestAdvancementPolicy\.trySacredGuardian[\s\S]*?WarriorAdvancementPolicy\.crossUnyielding\(target, hpBeforeEnemyHit, now\);\s*resolveEnemyDirectHitRecovery\(target, damage, stats\);/, 'party enemy attacks resolve fatal protection, threshold crossing, and recovery after actual health damage');
assert.match(source, /attacker\.currentHp = Math\.max\((?:0|getPlaytestHpFloor\(\)), attacker\.currentHp - actualCounterDamage\);\s*resolveEnemyDirectHitRecovery\(attacker, actualCounterDamage, counterStats, Math\.random, \{ triggerWasteland: false \}\);/g, 'enemy parry counters retain recovery handling but do not stack wasteland resilience from reflected damage');
...
assert.match(source, /ChapterThreeCraftedEpicAbilityPolicy\.getWastelandDamageReduction\(target, now\)[\s\S]*target\.currentHp = Math\.max\((?:0|getPlaytestHpFloor\(\)), target\.currentHp - damage\);[\s\S]*resolveEnemyDirectHitRecovery\(target, damage, stats\)/, 'wasteland resilience reduces the next hit before that direct hit adds or refreshes a stack');
