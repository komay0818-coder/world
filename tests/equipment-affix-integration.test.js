...
assert.match(source, /target\.currentHp = Math\.max\(0, target\.currentHp - damage\);\s*WarriorAdvancementPolicy\.crossUnyielding\(target, hpBeforeEnemyHit, now\);\s*resolveEnemyDirectHitRecovery\(target, damage, stats\);/, 'party enemy attacks resolve threshold crossing and recovery after actual health damage');
