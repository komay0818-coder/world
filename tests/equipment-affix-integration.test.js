...
assert.match(html, /script\.js\?v=20260911-stronghold-dungeon-v1/, 'the affix UI renderer uses the current local build');
...
assert.match(source, /target\.currentHp = Math\.max\(0, target\.currentHp - damage\);[\s\S]*?PriestAdvancementPolicy\.trySacredGuardian[\s\S]*?WarriorAdvancementPolicy\.crossUnyielding\(target, hpBeforeEnemyHit, now\);\s*resolveEnemyDirectHitRecovery\(target, damage, stats\);/, 'party enemy attacks resolve fatal protection, threshold crossing, and recovery after actual health damage');
