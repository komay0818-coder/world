...
assert.match(script, /const maxResource = getMaxCombatResourceForMember\(character, progress\)/, 'each party hunter derives a quiver capacity from their own equipment');
assert.match(script, /member\.resourceType === 'arrows' && !HunterArrowPolicy\.canUseSkill/, 'hunter skills skip when arrows are insufficient');
assert.match(script, /member\.resourceType !== 'arrows' && member\.resourceCurrent < cost/, 'other jobs keep their existing resource checks');
assert.match(script, /HunterArrowPolicy\.spendArrows\(member\.resourceCurrent/, 'successful hunter skills spend that member arrows');
...
assert.match(script, /member\.resourceType === 'arrows'[\s\S]*HunterArrowPolicy\.recoverArrows/, 'battle ticks recover each hunter arrows');
