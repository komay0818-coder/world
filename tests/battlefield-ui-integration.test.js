...
assert.match(html, /styles\/monster-slots\.css\?v=20260821-heavy-strike-v1/, 'battlefield loads the current local portrait styles');
assert.match(html, /script\.js\?v=20260821-heavy-strike-v1/, 'battlefield loads the current local combat logic');
...
assert.match(script, /art\.classList\.remove\('is-attacking', 'is-target-skill', 'is-area-skill', 'is-heavy-strike'\);[\s\S]*?void art\.offsetWidth;[\s\S]*?art\.classList\.add\(actionClass\);/, 'every attack reliably restarts its portrait animation');
...
assert.match(script, /const actionClass = skillId === 'heavy-strike' \? 'is-heavy-strike' : kind === 'basic' \? 'is-attacking' : area \? 'is-area-skill' : 'is-target-skill'/, 'attack movement types are separate');
...
assert.match(script, /showDamage: skill\.id !== 'heavy-strike'/, 'heavy strike suppresses the early generic damage number');
