...
assert.match(html, /e0d2a3b4c998500b1f4db11c633648d082cc2588\/styles\/monster-slots\.css/, 'battlefield loads the immutable equal enclosed portrait build');
...
assert.match(css, /--portrait-prototype-width: 156px;[\s\S]*?--portrait-prototype-height: 172px;/, 'normal monster and player prototypes share one frame size');
assert.match(css, /normal-portrait-prototype \.monster-image-frame,[\s\S]*?player-portrait-prototype\[aria-label\][\s\S]*?width: var\(--portrait-prototype-width\) !important;[\s\S]*?height: var\(--portrait-prototype-height\) !important;/, 'both prototype containers consume the same frame dimensions');
assert.match(css, /normal-portrait-prototype \.monster-slot-image[\s\S]*?inset: 26px 24px 30px !important;[\s\S]*?object-fit: contain !important;/, 'monster artwork remains fully enclosed by the frame');
assert.match(css, /player-portrait-prototype \.combat-unit-art[\s\S]*?inset: 26px 24px 30px !important;[\s\S]*?background-size: contain !important;/, 'player artwork remains fully enclosed by the frame');
