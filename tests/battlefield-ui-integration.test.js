...
assert.match(html, /script\.js\?v=20260903-chapter3-blue-crafted-stats-v1/, 'battlefield loads the current local combat logic');
...
assert.match(script, /kind === 'skill' && !area && \(targetRect \|\| targetAnchor\)[\s\S]*?classList\.add\('is-single-target-skill'\)/, 'single-target skills activate target-overlap travel from a live or captured target');
assert.match(script, /targetRect\.top \+ targetRect\.height \* \.55/, 'single-target travel resolves the live monster center');
assert.match(script, /targetPositions: targetAnchors/, 'single-target movement preserves its pre-damage target position');
assert.match(script, /targetAnchor \? fieldRect\.left \+ targetAnchor\.x/, 'a defeated or rerendered target falls back to its captured position');
assert.match(script, /kind === 'basic' && Number\(art\.combatAnimationLockUntil \|\| 0\) > Date\.now\(\)/, 'basic attacks cannot interrupt an active skill movement');
assert.match(script, /clearTimeout\(art\.combatAnimationTimer\)[\s\S]*?combatAnimationSequence/, 'each combat animation invalidates stale cleanup timers');
...
assert.match(script, /showDamage: !\['heavy-strike', 'whirlwind', 'charge', 'power-shot', 'multi-shot', 'piercing-shot', 'backstab', 'shadow-dance', 'poison-blade', 'fireball', 'blizzard', 'chain-lightning', 'holy-light', 'holy-nova'\]\.includes\(skill\.id\)/, 'preset damage numbers wait for their custom hit visuals');
...
assert.match(script, /'holy-light': \{ duration: 680, impactAt: 300, className: 'battle-effect-holy-light' \}/, 'holy light uses a short target-bound preset');
...
assert.match(script, /--holy-nova-x[\s\S]*?fieldRect\.width \* \.5[\s\S]*?--holy-nova-y/, 'priest moves to the battlefield center for holy nova');
assert.match(script, /holy-nova-gather[\s\S]*?holy-nova-ring[\s\S]*?--hit-delay/, 'holy nova creates one central ring and delayed target feedback');
assert.match(script, /Math\.hypot\(target\.anchor\.x - centerAnchor\.x, target\.anchor\.y - centerAnchor\.y\) \/ radius \* 400/, 'target timing derives from radial distance to the priest');
