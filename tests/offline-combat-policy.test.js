...
assert.deepEqual(survived, { died: false, defeated: 2, effectiveMs: 5000, deathAtMs: null, remainingHp: 50, potionsUsed: 0 }, 'simulation settles complete encounters and partial incoming damage');
...
const supplied = policy.simulate({ durationMs: 20000, player: { ...player, healingPotions: 5 }, monsters: [monster] });
assert.equal(supplied.died, false, 'automatic healing potions prevent an artificial offline defeat');
assert.equal(supplied.potionsUsed, 5);

const regenerating = policy.simulate({ durationMs: 20000, player: { ...player, hpRegeneration: 10 }, monsters: [monster] });
assert.equal(regenerating.died, false, 'passive health regeneration applies during offline combat');
