...
assert.match(index, /black-forest-depths-policy\.js\?v=20260911-depths-complete-v1/);
...
assert.match(index, /script\.js\?v=20260922-brokenrock-32-direct-v2/);
...
assert.match(script, /ForestAltarPolicy\.getDotEffect/);
assert.match(script, /function inflictForestAltarCorruptionFlame/);
assert.match(script, /playtest'\) !== 'forest-altar'/);
...
assert.match(script, /summonCount >= BlackForestDepthsPolicy\.BOSS\.rootSummonLimit/);
assert.match(script, /battle\.enemyTrailSummoned\?\.\[index\][\s\S]*battle\.enemyRespawns\[index\] = -1/,
  'defeated temporary roots leave the living roster without entering normal respawn');
assert.match(script, /blackForestDepthsAction === 'corrupted-heavy-axe'[\s\S]*blackstoneArmorBreakUntil/);
assert.match(script, /blackForestDepthsAction[\s\S]*withering-storm/);
assert.match(script, /params\.get\('playtest'\) !== 'deep-black-forest'/);
assert.match(script, /function completeChapterTwoFinalBattle/);
assert.match(script, /firstFinalBossKill[\s\S]*completeChapterTwoFinalBattle\(\)/);
assert.match(blackForestRenderer, /chapterTwoProgress\?\.completed[\s\S]*第二章已完成/);
