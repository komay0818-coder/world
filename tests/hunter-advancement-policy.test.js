...
assert.deepEqual(policy.applyGuardDamage(beastmaster, 1000, 1000, () => 0), { hunterDamage: 750, petDamage: 250, livingPets: 3, triggered: true, share: .25, petId: 'a', petUsesRemaining: 2, petDied: false, reviveAt: 0 });
policy.applyGuardDamage(beastmaster, 1000, 2000, () => 0);
assert.deepEqual(policy.applyGuardDamage(beastmaster, 1000, 3000, () => 0), { hunterDamage: 750, petDamage: 250, livingPets: 3, triggered: true, share: .25, petId: 'a', petUsesRemaining: 0, petDied: true, reviveAt: 33000 });
assert.equal(beastmaster.companions[0].alive, false);
assert.equal(policy.applyGuardDamage(beastmaster, 1000, 4000, () => 0).petId, 'b', 'dead pets cannot guard');
