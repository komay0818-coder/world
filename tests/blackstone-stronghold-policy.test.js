...
assert.equal(state.activeOutpostHp, 1200);
assert.equal(state.activeOutpostMaxHp, 1200);
let durability = policy.damageOutpost(state, 450, { now: 500, random: () => 0 });
assert.equal(durability.destroyed, false);
assert.equal(durability.state.activeOutpostHp, 750);
durability = policy.damageOutpost(durability.state, 750, { now: 500, random: () => 0 });
assert.equal(durability.destroyed, true);
assert.equal(durability.destroyedOutpostId, 'blackstone-supply-station');
assert.equal(durability.state.outpostActive, false);
state = policy.createState(() => 0);
