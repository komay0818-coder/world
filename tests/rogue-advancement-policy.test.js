...
assert.equal(policy.getEffect('blood-venom-rend', 6).ruptureDuration, 8);
assert.equal(policy.getEffect('blood-venom-rend', 6).ruptureTickInterval, 2);
assert.equal(policy.getEffect('blood-venom-rend', 6).ruptureTransferLimit, 2);
assert.deepEqual([1,2,3,4,5].map(level => policy.getEffect('blood-venom-rend', level).ruptureTransferLimit), [0,0,0,0,0]);
