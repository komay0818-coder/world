...
assert.deepEqual(policy.getTargetBonuses(assassin, [{ type: 'bleed', remaining: 3 }], {}, 1, 'basic', 2000), { damage: .20, crit: .10, criticalDamage: 0, dotDamage: 0, defenseReduction: 0, attackSpeed: .14, basicDamage: .15 });
...
assert.equal(policy.hasBleedingStatus([{ type: 'rupture', remaining: 1 }]), false);
assert.equal(policy.hasBleedingStatus([{ type: 'bleed', remaining: 1 }]), true);
...
assert.equal(policy.getPoisonDamageBonus(venom, [...sixPoison, ...sixBloodBleeds]), .60, 'blood venom erosion adds 35% at six poison and six bleed stacks');
...
assert.equal(settlement.dots.length, 4, 'settlement snapshots every current poison and bleed layer');
assert.deepEqual(policy.consumeSymbiosisLayers(dots, settlement), { poison: 2, bleed: 0 }, 'settlement consumes two layers while keeping at least one of each');
assert.equal(policy.poisonStacks(dots), 1);
assert.equal(policy.bloodBleedStacks(dots), 1);
assert.equal(policy.resolveSymbiosisSettlement(venom, dots, settlementState, true, 2999), null, 'settlement has a per-target two-second cooldown');
assert.ok(policy.resolveSymbiosisSettlement(venom, dots, settlementState, true, 3000), 'settlement is ready at the exact cooldown boundary');
const burstState = {};
assert.equal(policy.resolveBloodVenomBurst(venom, [...sixPoison, ...sixBloodBleeds], burstState, true, 1000), .10);
assert.equal(policy.resolveBloodVenomBurst(venom, [...sixPoison, ...sixBloodBleeds], burstState, true, 2999), 0, 'blood venom burst occurs at most once per target every two seconds');
assert.equal(policy.resolveBloodVenomBurst(venom, [...sixPoison, ...sixBloodBleeds], burstState, true, 3000), .10);
