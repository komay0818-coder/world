...
assert.match(index, /script\.js\?v=20260903-chapter3-blue-crafted-stats-v1/, 'the live page uses the current local game UI');
...
assert.match(script, /'spiritweave-crown': 'assets\/spiritweave-crown\.png'/, 'spiritweave crown artwork migrates into existing items');
assert.match(script, /'spiritweave-boots': 'assets\/spiritweave-boots\.png'/, 'spiritweave boots artwork migrates into existing items');
...
assert.match(script, /item\.kind === 'recipe' \|\| item\.kind === 'recipe-fragment' \|\| item\.kind === 'rune'\) return 'consumable'/, 'recipe, fragment and rune items appear in the existing backpack item category');
