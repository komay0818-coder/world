...
assert.deepEqual(chapterTwoCraft.item.baseStats, { defense: 7, hp: 55, dodge: .03 });
assert.equal(chapterTwoCraft.item.defense, 7);
assert.equal(chapterTwoCraft.item.hp, 55);
assert.equal(chapterTwoCraft.item.dodge, .03, 'chapter-two cloak base dodge enters the common equipment stat path');
assert.deepEqual(chapterTwoCraft.item.fixedAffixes.map((entry) => [entry.stat, entry.value]), [['maxHpPercent', 12], ['defensePercent', 12]]);
assert.equal(chapterTwoCraft.item.randomAffixes.length, 3);
assert.equal(chapterTwoCraft.item.randomAffixes.some((entry) => entry.id === 'defense_percent'), false, 'fixed defense cannot be rolled again');
const legacyChapterTwo = { kind: 'equipment', id: 'legacy', recipeId: chapterTwoRecipeId, templateId: chapterTwoRecipe.resultItemId, defense: 0, hp: 0, affixes: [{ id: 'max_hp_percent', value: 12 }] };
const migratedChapterTwo = CraftingPolicy.applyCraftedBaseStats(legacyChapterTwo);
assert.deepEqual(migratedChapterTwo.baseStats, { defense: 7, hp: 55, dodge: .03 });
assert.equal(migratedChapterTwo.affixes, legacyChapterTwo.affixes, 'base-stat compatibility does not reroll or replace existing affixes');
const chapterOneBeforeMigration = CraftingPolicy.generateCraftedEquipment('chapter1-green-wrist', { random: () => 0, instanceId: 'chapter-one-unchanged' });
assert.equal(CraftingPolicy.applyCraftedBaseStats(chapterOneBeforeMigration), chapterOneBeforeMigration, 'chapter-one crafted equipment is untouched');
