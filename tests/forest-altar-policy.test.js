...
assert.ok(policy.MONSTERS.every((monster) => monster.level === 23 && monster.stats !== null
  ...
  && monster.skillIds.length === 2 && monster.aiProfileId !== null && monster.implemented === true));
...
assert.deepEqual(policy.getDotEffect('corruption-flame'), { effectName: '腐化之焰', attackRatio: .25, tickMs: 1000, durationMs: 5000 });
assert.equal(policy.getDotEffect('corrupted-bite'), null);
