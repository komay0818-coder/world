...
assert.equal(policy.getMap('spider-nest').implemented, true);
assert.equal(policy.getMap('spider-nest').contentStatus, 'combat-ready');
...
assert.equal(stronghold.background, 'assets/blackstone-stronghold-background.png');
const strongholdBackground = fs.readFileSync(path.join(__dirname, '..', stronghold.background));
assert.equal(strongholdBackground.subarray(1, 4).toString(), 'PNG', 'the Blackstone Stronghold background is a PNG asset');
assert.equal(stronghold.enemyPoolId, 'blackstone-stronghold-enemies');
assert.equal(stronghold.bossId, 'blackstone-warlord');
...
assert.equal(policy.getMap('black-forest-depths').contentStatus, 'combat-ready');
