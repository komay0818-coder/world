...
assert.match(source, /skill\.id === 'poison-blade'[\s\S]*stats\.attack \* \.18 \* stats\.dotMultiplier[\s\S]*tickIntervalMs: 2000/);
...
assert.match(source, /ruptureDamage[\s\S]*applyRogueDotEntryDamage\(target\.index, member, 'rupture'/, 'rupture deals its separate half-tick entry damage');
...
assert.match(source, /function applyPendingRoguePlagueSpread[\s\S]*consumePlague[\s\S]*applyRoguePoison\(index, member/);
