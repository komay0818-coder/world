...
assert.deepEqual(policy.MAP_DROP_CONFIGS['wolf-den'].map(({ dropRate }) => dropRate), [.25]);
...
assert.deepEqual(allWolfDrops.map(({ id }) => id), ['building-wolf-fur'], 'building policy no longer rolls a duplicate wolf fang');
...
assert.deepEqual(policy.LEVEL_TWO_COSTS.alchemy.materials, { 'wolf-fang': 50, 'building-boar-tusk': 50, 'building-black-ore': 50 });
assert.deepEqual(policy.LEVEL_TWO_COSTS.rune.materials, { 'building-black-ore': 250, 'wolf-fang': 25, 'building-boar-tusk': 25 });
...
assert.ok(buildingIds.filter((id) => id !== 'wolf-fang').every((id) => id.startsWith('building-')), 'only wolf fang is shared with crafting');
assert.equal(new Set(buildingIds).size, buildingIds.length);
assert.equal(policy.getMaterial('wolf-fang').name, '狼牙');
assert.deepEqual(policy.normalizeWolfFangInventory([{ id: 'wolf-fang', quantity: 3 }, { id: 'building-wolf-fang', quantity: 2 }]).map(({ id, quantity }) => ({ id, quantity })), [{ id: 'wolf-fang', quantity: 5 }]);
