...
const racialCompanions = {
  human: { icon: 'assets/hunter-companion-human-icon.png', portrait: true, name: '王國獵犬' },
  elf: { icon: 'assets/hunter-companion-elf-icon.png', portrait: true, name: '月光山貓' },
  orc: { icon: 'assets/hunter-companion-orc-icon.png', portrait: true, name: '獠牙戰狼' },
  undead: { icon: 'assets/hunter-companion-undead-icon.png', portrait: true, name: '亡靈獵犬' }
};
...
const monsterVisualScaleCorrections = {
  ...
  'skullcrusher-berserker': 1.007,
  'skullcrusher-shieldguard': .969,
  'skullcrusher-hunter': .94,
  'skullcrusher-shaman': .987,
  'skullcrusher-centurion': 1.092,
  'skullcrusher-vanguard-commander': 1.008,
...
function getProgress(activeCharacterOverride = null) {
  ...
  if (saved.chapterThreeBlueCraftedBaseStatsMigrationVersion !== 'chapter3-blue-crafted-base-stats-v1') {
    saved.inventory = (Array.isArray(saved.inventory) ? saved.inventory : []).map(CraftingPolicy.applyCraftedBaseStats);
    saved.equipment = Object.fromEntries(Object.entries(saved.equipment || {})
      .map(([slot, item]) => [slot, CraftingPolicy.applyCraftedBaseStats(item)]));
  ...
  if (saved.chapterThreeSpecialEquipmentTemplateMigrationVersion !== 'chapter3-special-equipment-template-v1') {
    saved.inventory = (Array.isArray(saved.inventory) ? saved.inventory : []).map(ChapterThreeSpecialEquipmentPolicy.upgradeSpecialEquipmentInstance);
    saved.equipment = Object.fromEntries(Object.entries(saved.equipment || {})
      .map(([slot, item]) => [slot, ChapterThreeSpecialEquipmentPolicy.upgradeSpecialEquipmentInstance(item)]));
    saved.chapterThreeSpecialEquipmentTemplateMigrationVersion = 'chapter3-special-equipment-template-v1';
    localStorage.setItem('stardust-progress', JSON.stringify(saved));
  }
  ...
  const normalizedProgress = {
    ...
    chapterThreeProgress: ChapterThreeProgressionPolicy.createDefaultState(),
    unlockedChapter: 1,
...
function claimOfflineRewards() {
  ...
  const simulation = OfflineCombatPolicy.simulate({
    ...
    player: {
      ...
      attack: stats.attack,
      defense: Math.round(stats.defense * ChapterThreeSpecialEquipmentPolicy.getDefenseMultiplier(getMainBattleMember(), now)),
...
function renderCharacterAbilities() {
  ...
  document.querySelector('#inventory-content').innerHTML = `
    <section class="ability-summary">
      ...
      ${advancementPolicy && !progress.advancedClass ? `<section class="workshop-section"><h3>Lv45 第一次轉職</h3><p>條件：Lv45＋英雄之證Ⅰ、Ⅱ、Ⅲ。</p>${Object.values(advancementPolicy.ADVANCED_CLASSES).map((entry) => `<button type="button" data-first-advance="${entry.id}" ${advancementPolicy.canAdvance(character, progress) ? '' : 'disabled'}>轉職為${entry.name}</button>`).join('')}</section>` : ''}
    </section>`;
...
function getDropLookupItems() {
  return DropLookupPolicy.buildIndex({
    ...
    recipePolicies: [ChapterOneRecipeDropPolicy, ChapterTwoRecipeDropPolicy, ChapterThreeRecipeDropPolicy],
...
function renderMapSelector() {
  ...
  document.querySelector('#inventory-content').innerHTML = `<section class="map-selection-grid">${maps.map((map) => {
    ...
    const detail = isRegionHub
      ? map.chapter === 3
        ? `<em>${unlocked ? '第三章已解鎖・3-1 赤岩荒原已開放' : '完成第二章後解鎖'}</em>`
        : map.id === 'black-forest'
...
function renderRedrockWastesRegions() {
  const progress = getProgress();
  const activeMap = getActiveMap(progress);
  const modal = document.querySelector('#inventory-modal');
  document.querySelector('#inventory-title').textContent = '第三章・赤岩荒原';
  document.querySelector('#inventory-content').innerHTML = `
    <button type="button" class="map-region-back" data-map-region-back>← 返回地區選擇</button>
    <section class="region-overview-card">
      <div><b>赤岩荒原</b><small>第三章</small></div>
      <em>${ChapterThreeMapPolicy.CHAPTER.summary}</em>
    </section>
    <section class="map-region-grid">${ChapterThreeMapPolicy.MAPS.map((region, index) => {
      const status = ChapterThreeProgressionPolicy.getMapState(progress, region.id, region.implemented);
...
function selectAdventureMap(mapId) {
  ...
  if (map.chapter === 3 && !ChapterThreeProgressionPolicy.canEnter(progress, map.id, map.implemented)) return;
...
function applyRoguePoison(index, member, damage, duration, maxStacks, options = {}) {
  if (RogueAdvancementPolicy.isAdvanced(member?.progress, 'venom')) maxStacks = Math.max(maxStacks, RogueAdvancementPolicy.getPoisonMaxStacks(member));
  ...
  if (added && RogueAdvancementPolicy.isAdvanced(member?.progress, 'venom')) applyRogueDotEntryDamage(index, member, 'poison', damage * added, RogueAdvancementPolicy.POISON_ENTRY_RATIO, options.now || Date.now());
  return added;
}
...
function getSkillResourceCost(job, skill) {
  ...
  const rogueSkill = RogueAdvancementPolicy.getSkill(skill?.id);
  if (rogueSkill) return rogueSkill.energyCost || 0;
...
function getSkillDescription(job, skill) {
  ...
  if (effect.shockedVulnerability) parts.push(`感電易傷 ${effect.shockedDuration || 4} 秒：施法者本人後續所有傷害 ×${1 + effect.shockedVulnerability}`);
...
function renderSkillDetailModal() {
  ...
  const cooldown = skill.type === 'active' ? Math.round((getSkillEffect(progress, character.job, skill).cooldown || skill.cooldown) * skillCooldownMultiplier) : 0;
...
function rewardVictory(index) {
  ...
  recipeDrops.push(...ChapterThreeRecipeDropPolicy.grantRecipeDrops(progress, enemy, currentMap.id));
  ...
  if (currentMap.chapter === 3 && enemy.isBoss) {
...
function getEnemySkillState(index) {
  ...
  battle.enemySkillStates[index] = battle.enemySkillStates[index] || {
    ...
    paralyzedUntil: 0, shockedByMage: {}
  };
  return battle.enemySkillStates[index];
}

function getRedrockEnemyState(index, currentHp = null, maxHp = null) {
  battle.redrockEnemyStates = Array.isArray(battle.redrockEnemyStates) ? battle.redrockEnemyStates : [];
  battle.redrockEnemyStates[index] = battle.redrockEnemyStates[index] || RedrockWastesPolicy.createState(Date.now());
  if (currentHp !== null && maxHp !== null) RedrockWastesPolicy.updateThresholds(battle.enemyTypes[index], battle.redrockEnemyStates[index], currentHp, maxHp);
  return battle.redrockEnemyStates[index];
}
...
function getMageShockState(index, member) {
  const state = getEnemySkillState(index), key = getMageShockKey(member);
  state.shockedByMage = state.shockedByMage || {};
  return key ? state.shockedByMage[key] || null : null;
}

function applyEnemySkillState(index, effect, now = Date.now(), source = null) {
  ...
  if (effect.shockedVulnerability && source?.job === 'mage') {
    const key = getMageShockKey(source), previous = key ? state.shockedByMage?.[key] : null, stats = MageAdvancementPolicy.telemetry(source);
    if (previous?.until > now) stats.shockedRefreshes++; else stats.shockedApplications++;
    state.shockedByMage = state.shockedByMage || {};
    if (key) state.shockedByMage[key] = { until:now+(effect.shockedDuration||4)*1000,generation:(previous?.generation||0)+1,extended:false };
  }
}
...
function extendElementalCollapseStatuses(index, member, now = Date.now()) {
  ...
  const shock = getMageShockState(index, member);
  if (!burn || !(now < state.slowedUntil || now < state.frozenUntil) || !(now < state.paralyzedUntil || shock?.until > now)) return false;
  ...
  if (shock?.until > now) {
    if (!shock.extended) { shock.until += 1000; shock.extended = true; stats.shockedExtendedSeconds++; }
...
function applyDamageToMonster(index, baseDamage, profile, options = {}) {
  ...
  const shockedVulnerabilityMultiplier = getMageShockState(index, attacker)?.until > now ? 1.5 : 1;
  ...
  if (shockedVulnerabilityMultiplier > 1 && attacker?.job === 'mage') {
...
function applyElementalStormStatus(index, element, member, damage, now) {
  const state = getEnemySkillState(index);
  if (element === 'fire') {
    const shockSnapshot = getMageShockState(index,member)?.until > now ? 1.5 : 1;
    applyDot(index, 'burn', Math.max(1, Math.ceil(damage / shockSnapshot * .18 * member.stats.dotMultiplier)), 4, 1, { source: member });
...
function useAutoSkillForMember(member, now = Date.now()) {
  ...
  for (const skill of unlocked.filter((entry) => ['light-fountain','guardian-sanctuary'].includes(entry.id))) {
    ...
    if (member.resourceCurrent < cost) { CombatCorePolicy.recordResourceBlock(member, skill.id, member.resourceCurrent); continue; }
    CombatCorePolicy.clearResourceBlock(member, skill.id);
    ...
    if (skill.id === 'light-fountain') livingAllies.forEach((ally) => {
      ...
      PriestAdvancementPolicy.heal(member, ally, amount, 'light-fountain');
      if (ratio < .4 && effect.lowHpShield) { PriestAdvancementPolicy.grantShield(member, ally, ally.maxHp * effect.lowHpShield, 'light-fountain', now, effect.shieldDuration * 1000); ally.lightFountainShieldUntil = now + effect.shieldDuration * 1000; }
    });
    ...
    const resourceBeforeSupportCost=member.resourceCurrent;member.resourceCurrent-=cost;const supportSpent=Math.max(0,resourceBeforeSupportCost-member.resourceCurrent);CombatCorePolicy.record(member,'resourceSpent',supportSpent);CombatCorePolicy.recordEvent(member,'resourceEvents',{atMs:now,type:'spend',amount:supportSpent,current:member.resourceCurrent,skill:skill.id});CombatCorePolicy.recordSkillCast(member,skill.id,{atMs:now,targetIndexes:[],resourceBefore:resourceBeforeSupportCost,resourceAfter:member.resourceCurrent});member.globalSkillReadyAt=now+1000; member.skillCooldowns[skill.id]=now+effect.cooldown*1000/stats.cooldownSpeed;
  ...
  for (const skill of attackSkills) {
    ...
    if (member.resourceType !== 'arrows' && member.resourceCurrent < cost) {
      if (member.resourceType === 'energy' || member.resourceType === 'mana') CombatCorePolicy.recordResourceBlock(member, skill.id, member.resourceCurrent);
      continue;
    }
    if (member.resourceType === 'energy' || member.resourceType === 'mana') CombatCorePolicy.clearResourceBlock(member, skill.id);
    ...
    if (skill.id === 'corrosive-strike') {
      RogueAdvancementPolicy.applyCoating(member, getSkillUpgradeLevel(progress, member.job, skill), now);
      const resourceBeforeSkillCost = member.resourceCurrent;
      ...
      CombatCorePolicy.recordSkillCast(member, skill.id, { atMs: now, targets: [], damage: 0, critical: false });
      CombatCorePolicy.record(member, 'resourceSpent', actualResourceSpent);
      CombatCorePolicy.recordEvent(member, 'resourceEvents', { atMs: now, type: 'spend', amount: actualResourceSpent, current: member.resourceCurrent, skill: skill.id });
      member.skillCooldowns[skill.id] = now + (skillEffect.cooldown || skill.cooldown) * 1000;
      member.globalSkillReadyAt = now + 1000;
      logBattle(`☠ ${member.name}施放【${skill.name}】，為主手武器淬毒。`, 'system');
      return true;
    }
    ...
    if (skill.id === 'holy-nova' && hits.length) {
      PriestAdvancementPolicy.heal(member,member,member.maxHp*(skillEffect.selfHealPerTarget||0)*hits.length,'holy-nova');
      if (skillEffect.shield && hits.length >= skillEffect.shieldAtTargets) PriestAdvancementPolicy.grantShield(member,member,member.maxHp*skillEffect.shield,'holy-nova',now);
    }
  ...
  if (!healSkill || healTarget.currentHp / healTarget.maxHp > .7) return false;
  if (member.resourceCurrent < getSkillManaCost(healSkill)) { CombatCorePolicy.recordResourceBlock(member, healSkill.id, member.resourceCurrent); return false; }
  ...
  if (graceTriggered && grace.spread && actualHeal > 0) {
    (battle.partyMembers || []).filter((ally) => ally.alive && ally.id !== healTarget.id).slice(0, grace.maxAllies).forEach((ally) => {
      if(member.job==='priest')PriestAdvancementPolicy.heal(member,ally,actualHeal*grace.spread,'divine-grace');else ally.currentHp = Math.min(ally.maxHp, ally.currentHp + actualHeal * grace.spread);
    });
  }
  ...
  CombatCorePolicy.record(member,'resourceSpent',actualHealResourceSpent);CombatCorePolicy.recordEvent(member,'resourceEvents',{atMs:now,type:'spend',amount:actualHealResourceSpent,current:member.resourceCurrent,skill:healSkill.id});CombatCorePolicy.recordSkillCast(member,healSkill.id,{atMs:now,targetIds:[healTarget.id],resourceBefore:resourceBeforeHealCost,resourceAfter:member.resourceCurrent});
  ...
  if (healEffect.afterglow && actualHeal > 0) setTimeout(() => {
    ...
    if(member.job==='priest')PriestAdvancementPolicy.heal(member,healTarget,Math.ceil(actualHeal*healEffect.afterglow),'afterglow');else healTarget.currentHp = Math.min(healTarget.maxHp, healTarget.currentHp + Math.ceil(actualHeal * healEffect.afterglow));
    if (fighting) updateBattleUI();
  }, 3000);
...
function updatePartyMemberResource(member, now) {
  ...
  PriestAdvancementPolicy.expireShields(member,now);
  if (usesManaResource(member.job)) {
    ...
    const actualRecovery=Math.max(0,member.resourceCurrent-resourceBeforeRecovery);CombatCorePolicy.record(member, 'resourceRecovered', actualRecovery);if(actualRecovery)CombatCorePolicy.recordEvent(member,'resourceEvents',{atMs:now,type:'natural',amount:actualRecovery,current:member.resourceCurrent});
    updatePartyMemberManaExhaustion(member);
...
function processPartyMemberAttacks(now = Date.now()) {
  for (const member of battle.partyMembers || []) {
    ...
    const coatingExecution = RogueAdvancementPolicy.getCoatingExecution(member, RogueAdvancementPolicy.poisonStacks(battle.enemyDots[targetIndex]), now);
    ...
    if (!result.evaded && result.finalDamage > 0 && coatingExecution && battle.enemyHps[targetIndex] > 0) {
      if (coatingExecution.bonusPower > 0) {
        const poisonMultiplier = 1
          + RogueAdvancementPolicy.getPoisonDamageBonus(member, battle.enemyDots[targetIndex])
          + RogueAdvancementPolicy.getTargetBonuses(member, battle.enemyDots[targetIndex], getEnemySkillState(targetIndex), battle.enemyHps[targetIndex] / getEnemyDefinition(targetIndex).maxHp, 'dot', now).dotDamage;
        applyDamageToMonster(targetIndex, member.stats.attack * coatingExecution.bonusPower * coatingExecution.bonusMultiplier * member.stats.dotMultiplier * poisonMultiplier, { damageType: 'periodic', attackRange: 'none' }, { attacker: member, attackKind: 'coating-poison', sourceSkill: 'coating-poison', canEvade: false, canParry: false, effectType: 'dot' });
      }
    ...
    if (!result.evaded) {
      ...
      if (member.job === 'assassin' && battle.enemyHps[targetIndex] > 0) {
        ...
        const masteryChance = (mastery.offhandChance || 0) + (lethalExecution?.extraOffhandChance || 0);
...
function ensureHunterCompanions(member, now = Date.now()) {
  ...
  member.companions.forEach((pet) => HunterAdvancementPolicy.syncPetGuard(member, pet));
...
function processHunterCompanionAttacks(now = Date.now()) {
  for (const member of battle.partyMembers || []) {
    ...
    (member.companions || []).filter((pet) => deadBeforeUpdate.has(pet.id) && pet.alive !== false).forEach((pet) => {
      CombatCorePolicy.recordEvent(member, 'petEvents', { atMs: now, petId: pet.id, kind: 'pet-revive', guardUses: pet.currentGuardUses });
    });
    const bonuses = HunterAdvancementPolicy.getPetBonuses(member, now);
    for (const pet of ensureHunterCompanions(member, now)) {
      if (pet.alive === false) continue;
...
function legacyEnemyAttackTick() {
  ...
  for (const attackingEnemyIndex of aliveEnemyIndexesByAge()) {
    ...
    if (mainMember) {
      ...
      ChapterThreeSpecialEquipmentPolicy.resolveEnemyAttackOutcome(mainMember, { actualDamage: enemyHit, parried, dodged }, now);
...
function enemyAttackTick() {
  ...
  for (const enemyIndex of aliveEnemyIndexesByAge()) {
    ...
    const redrockState = getActiveMap(progress).id === 'redrock-wastes-entrance'
      ? getRedrockEnemyState(enemyIndex, enemyCurrentHp, enemy.maxHp) : null;
    const redrockAction = redrockState
    ...
    let damage = dodged ? 0 : MonsterDefense.resolvePlayerDamage({
      baseDamage: rawDamage,
      defense: Math.max(0, Math.round(stats.defense * ChapterThreeSpecialEquipmentPolicy.getDefenseMultiplier(target, now) * armorBreakMultiplier * spiderNestArmorMultiplier * piercingMultiplier)),
      damageReduction: Math.min(.9, stats.damageReduction
        + ChapterTwoSpecialEquipmentPolicy.getIncomingDamageReduction(target.progress.equipment, target.currentHp / target.maxHp)
        + ChapterThreeCraftedEpicAbilityPolicy.getWastelandDamageReduction(target, now)
        + WarriorAdvancementPolicy.getUnyieldingReduction(target, now)
    ...
    if (target.currentHp > 0 && target.currentHp / target.maxHp < .3 && target.job === 'priest' && target.level >= 8 && now >= (target.holyProtectionReadyAt || 0)) {
      ...
      PriestAdvancementPolicy.grantShield(target,target,target.maxHp * protection.shield,'holy-protection',now,6000);
    ...
    if (!dodged && damage > 0 && redrockAction?.dot) {
      const dot = RedrockWastesPolicy.createDot(redrockAction.id, enemy.id, enemy.attack);
      target.bleed = { effectName: redrockAction.name, tickMs: dot.tickMs, tickDamage: Math.max(1, Math.ceil(dot.damage)), nextTickAt: now + dot.tickMs, expiresAt: now + dot.durationMs };
      RedrockWastesPolicy.recordDotApplication(redrockState, redrockAction.id);
      logBattle(`🩸【${enemy.name}】施放【${redrockAction.name}】，${target.name}陷入持續傷害。`, 'system');
    }
    if (!dodged && damage > 0 && redrockAction?.debuff?.type === 'defense-down') {
      target.redrockDefenseDown = redrockAction.debuff.value;
      target.redrockDefenseDownUntil = now + redrockAction.debuff.durationMs;
      RedrockWastesPolicy.recordDebuff(redrockState, redrockAction.id);
      logBattle(`🎯【${enemy.name}】施放【${redrockAction.name}】，${target.name}防禦降低 15%。`, 'system');
    }
    if (!dodged && damage > 0 && redrockAction?.stunChance) {
      const stun = RedrockWastesPolicy.resolveStun(redrockState, Math.random());
      if (stun) applyControlEffectToPlayer(target, { type: 'stun', baseDurationMs: stun.durationMs, source: redrockAction.name }, now);
    }
    if (!dodged && damage > 0 && redrockAction && !redrockAction.passive && !redrockAction.dot && !redrockAction.debuff) {
...
document.querySelector('#inventory-modal').addEventListener('click', (event) => {
  ...
  if (regionHubButton) {
    ...
    else if (regionHubButton.dataset.openMapRegion === 'redrock-wastes') renderRedrockWastesRegions();
