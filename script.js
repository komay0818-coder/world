...
function getProgress() {
  ...
  if (saved.equipmentVisualMigrationVersion !== 'chapter-two-greatsword-images-v5') {
    ...
    saved.equipmentVisualMigrationVersion = 'chapter-two-greatsword-images-v5';
    localStorage.setItem('stardust-progress', JSON.stringify(saved));
  }
...
function processPartyMemberAttacks(now = Date.now()) {
  for (const member of battle.partyMembers || []) {
    ...
    const trailSlowMultiplier = now < (member.blackstoneAttackSpeedPenaltyUntil || 0)
      ...
      : 1;
    PartyPolicy.scheduleNextAttack(member, now, member.attackSpeed, exhaustedMultiplier * trailSlowMultiplier);
  }
}
...
function enemyAttackTick() {
  ...
  for (const enemyIndex of aliveEnemyIndexesByAge()) {
    ...
    const rawDamage = getMonsterAttackPower(enemy, progress, enemyCurrentHp) * activeDamageMultiplier * markedHumanBonus * markedArcherBonus * (critical ? 1.5 : 1);
    const parried = !dodged && Math.random() < stats.parry;
